...
<details>
<summary><b>要花钱吗？会动我的账号吗？</b></summary>
...
<details>
<summary><b>它替你挡了哪三个坑</b></summary>

- **B 站有两个取字幕的接口，其中一个会返回别的视频的字幕**——不是截断，是整份串号，同一个视频连查三次拿到三份不相干的文稿。不少现成工具用的正好是坏的那个。
- **mp4 完不完整，光比时长查不出来**：截断到 60% 的文件，时长照样报满格，用 ffmpeg 过一遍也不报错。只有比对文件末尾那一帧的位置才看得出来。所以它下完会自己验一遍，坏的删掉重下。
- **中文 PDF 的可搜索性很脆弱**：不少中文字体会把常用字映射到「康熙部首」那个偏门编码区，看着正常，一搜什么都搜不到。这个工具自带验过的字体。

实测数据在 [references/](skills/digesting-bilibili-videos/references/)。
